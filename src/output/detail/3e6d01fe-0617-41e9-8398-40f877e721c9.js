export default {
  "completion": {
    "percentage": 72,
    "ChinaReadiness": 58,
    "EnterpriseGuardrails": 50,
    "businessSupport": 68,
    "dataManagement": 32,
    "dependencies": 100,
    "information": 88,
    "projects": 100,
    "robots": 100,
    "sourcing": 53
  },
  "subscriptions": [
    {
      "firstName": "Carina",
      "lastName": "Lillieqvist",
      "displayName": "Carina Lillieqvist",
      "email": "carina.lillieqvist@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "IT Delivery Responsible"
      ]
    },
    {
      "firstName": "Tom",
      "lastName": "Varis",
      "displayName": "Tom Varis",
      "email": "tom.varis@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "Product Owner"
      ]
    },
    {
      "firstName": "David",
      "lastName": "Aslett",
      "displayName": "David Aslett",
      "email": "david.aslett@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "Information Owner"
      ]
    },
    {
      "firstName": "Mangesh",
      "lastName": "Pawar",
      "displayName": "Mangesh Pawar",
      "email": "mangesh.pawar@scania.com",
      "type": "RESPONSIBLE",
      "role": []
    },
    {
      "firstName": "Hans",
      "lastName": "Öhrström",
      "displayName": "Hans Öhrström",
      "email": "hans.ohrstrom@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "Architect"
      ]
    },
    {
      "firstName": "Gizem Plamenova",
      "lastName": "Davidova",
      "displayName": "Gizem Plamenova Davidova",
      "email": "gizem.plamenova.davidova@scania.com",
      "type": "RESPONSIBLE",
      "role": []
    }
  ],
  "information": {
    "name": "Nile China",
    "description": "Nile Packaging China – is the same application as Nile used in Europe with a separate database and integration changes required due to hosting solution and region.",
    "lifecycle": {
      "asString": "active",
      "phases": [
        {
          "phase": "active",
          "startDate": "2019-12-16",
          "milestoneId": null
        }
      ]
    },
    "productCategory": "",
    "predecessor": [
      {
        "id": "4bc43ce5-ff66-465c-b814-f2bf74f0ad93",
        "displayName": "EMBASY",
        "description": "Administrates Scania Packaging Material. Embasy is the application that suppliers will use to book packaging material from Scania. ",
        "type": "Application",
        "tags": [
          "Not China affected",
          "Maturity Level 1"
        ],
        "subscriptions": []
      }
    ],
    "successor": []
  },
  "dependencies": {
    "parent": [],
    "children": []
  },
  "initiatives": [
    {
      "id": "ab32fafe-70d0-420c-86bb-e322e0164435",
      "displayName": "SAS Industrial Project - China",
      "fullName": "SAS Industrial Project - China",
      "tags": [
        "China affected"
      ],
      "lifecycle": {
        "asString": "active",
        "phases": [
          {
            "phase": "active",
            "startDate": "2021-09-01"
          }
        ]
      },
      "subscriptions": [
        {
          "displayName": "Anton Smith",
          "email": "anton.smith@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Enno Grüning",
          "email": "enno.gruning@scania.com",
          "type": "OBSERVER"
        }
      ]
    },
    {
      "id": "c78bc66a-aaef-45c0-b4f9-2eb54e27c20a",
      "displayName": "SAS Industrial Project - China / China Logistics IT",
      "fullName": "China Logistics IT",
      "tags": [
        "China affected"
      ],
      "lifecycle": {
        "asString": "active",
        "phases": [
          {
            "phase": "plan",
            "startDate": "2022-04-01"
          },
          {
            "phase": "phaseIn",
            "startDate": "2022-07-01"
          },
          {
            "phase": "active",
            "startDate": "2022-09-01"
          }
        ]
      },
      "subscriptions": [
        {
          "displayName": "Tove Norén",
          "email": "tove.noren@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Anton Smith",
          "email": "anton.smith@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Sivamani Karuppasamy",
          "email": "sivamani.karuppasamy@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Tommy Dahlgren",
          "email": "tommy.dahlgren@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Renato Colado",
          "email": "Renato.Colado@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Filip Eriksson",
          "email": "filip.eriksson@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Joanna Pochec",
          "email": "joanna.pochec@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Marcelo Colovato",
          "email": "marcelo.colovato@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Burak Gavgacioglu",
          "email": "burak.gavgacioglu@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Samuel Suderbys",
          "email": "samuel.suderbys@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Henrik Boman",
          "email": "henrik.boman@scania.com",
          "type": "RESPONSIBLE"
        }
      ]
    },
    {
      "id": "fc409769-43d5-4b5d-a5ba-62150645cad8",
      "displayName": "SAS Industrial Project - China / China Logistics IT / China packaging",
      "fullName": "China packaging",
      "tags": [
        "China affected"
      ],
      "lifecycle": {
        "asString": "active",
        "phases": [
          {
            "phase": "plan",
            "startDate": "2022-06-01"
          },
          {
            "phase": "phaseIn",
            "startDate": "2022-07-01"
          },
          {
            "phase": "active",
            "startDate": "2022-09-01"
          }
        ]
      },
      "subscriptions": [
        {
          "displayName": "Tove Norén",
          "email": "tove.noren@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Tom Varis",
          "email": "tom.varis@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Renato Nascimento",
          "email": "renato.nascimento@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Emma Lindgren",
          "email": "emma.lindgren@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Joanna Pochec",
          "email": "joanna.pochec@scania.com",
          "type": "OBSERVER"
        }
      ]
    },
    {
      "id": "c22a18c3-fcaf-4283-9f95-d3594d2d9af4",
      "displayName": "SAS Industrial Project - China / SAS Final Assembly IT",
      "fullName": "SAS Final Assembly IT",
      "tags": [
        "China affected",
        "Maturity Level 2"
      ],
      "lifecycle": {
        "asString": "active",
        "phases": [
          {
            "phase": "plan",
            "startDate": "2022-04-01"
          },
          {
            "phase": "phaseIn",
            "startDate": "2022-07-01"
          },
          {
            "phase": "active",
            "startDate": "2022-09-01"
          }
        ]
      },
      "subscriptions": [
        {
          "displayName": "Frédéric Anquetil",
          "email": "Frederic.Anquetil@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Douglas Mitsuru Higa",
          "email": "Douglas.Higa@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Anton Smith",
          "email": "anton.smith@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Thomas Sahleström",
          "email": "thomas.sahlestrom@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Joanna Pochec",
          "email": "joanna.pochec@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Arthur Takaki",
          "email": "arthur.takaki@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Pallavi Shrotri",
          "email": "pallavi.shrotri@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Claes Boije",
          "email": "claes.boije@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Torbjörn Landenberg",
          "email": "torbjorn.landenberg@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Henrik Bylund",
          "email": "henrik.bylund@scania.com",
          "type": "OBSERVER"
        },
        {
          "displayName": "Pekka Palonen",
          "email": "pekka.palonen@scania.com",
          "type": "OBSERVER"
        }
      ]
    }
  ],
  "businessSupport": {
    "functionFit": {
      "businessCriticality": "businessCritical",
      "functionalSuitability": "insufficient",
      "iramNecessary": "yes",
      "lifecycle": [
        null,
        null
      ]
    },
    "capabilities": [
      {
        "id": "6a4b2bfc-6418-451f-b5f2-45ba20f1ecbf",
        "displayName": "H. Logistics and Warehouse Management / Warehouse Management / Packaging Management",
        "description": "The required processes to ship packaging to relevant organizations (suppliers mostly). Storage, sorting, recycling, breakdown, ordering, shipping and material planning for packaging numbers",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2018-09-13"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "China affected"
        ]
      }
    ],
    "processes": [
      {
        "id": "ce41924b-2e77-4a67-b82e-32668035b5bb",
        "displayName": "Order to delivery",
        "description": "Owner: Martin Lyckström",
        "lifecycle": null,
        "subscriptions": [],
        "tags": [
          "Core process"
        ]
      },
      {
        "id": "73e10cd3-428c-45ba-b4c4-ac2c4927047f",
        "displayName": "Order to delivery / Major support processes within Production & Logistics / External Logistics",
        "description": null,
        "lifecycle": null,
        "subscriptions": [],
        "tags": []
      },
      {
        "id": "9e9583f5-b85a-43a6-94bd-ec493662a28d",
        "displayName": "Order to delivery / Packaging",
        "description": null,
        "lifecycle": null,
        "subscriptions": [],
        "tags": []
      }
    ],
    "organisations": [
      {
        "id": "2c7dee79-228f-4a36-abd9-227979ae03a8",
        "displayName": "Scania - Scania CV AB / I - Scania IT / IN - Order to Delivery IT / INA - Logistics IT / INAD - Supply Chain Networks IT",
        "usageType": "ITmaintenance",
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-02-08"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "From IGA"
        ]
      },
      {
        "id": "aa2b5483-c783-4f50-9cf1-8ab1d83c8d0c",
        "displayName": "Scania - Scania CV AB / P_PL - Production & Logistics / O - Logistics / ON - Logistics Centre Netherlands",
        "usageType": "user",
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2020-09-04"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "From IGA"
        ]
      },
      {
        "id": "c720b516-976e-4ba2-87fb-171db95d2289",
        "displayName": "Scania - Scania CV AB / P_PL - Production & Logistics / O - Logistics / OS - Strategy & Business Development / OSP - Packaging Development",
        "usageType": "owner",
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-09-01"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "From IGA"
        ]
      },
      {
        "id": "62bfe281-2832-4d08-8cbf-450b15fb6c16",
        "displayName": "Scania - Scania CV AB / P_PL - Production & Logistics / O - Logistics / OS - Strategy & Business Development / OSP - Packaging Development / OSPA - Network & Business Development",
        "usageType": "owner",
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-09-01"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "From IGA"
        ]
      }
    ]
  },
  "management": {
    "objects": [
      {
        "id": "4065cbad-369d-4d29-b18d-c84af3649dd6",
        "description": "packaging for handling is the quantity of packaging arrived at packaging handling center that needs to be handled",
        "displayName": "Logistics Domain / Packaging Capability Entity / Packaging For Handling",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      },
      {
        "id": "2ad8337e-e140-4c91-87db-0fa894601ffb",
        "description": "packaging handling center is a type of organisation that provides packaging logistics services.",
        "displayName": "Logistics Domain / Packaging Capability Entity / Packaging Handling Center",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      },
      {
        "id": "54fd1c88-c2bb-4f61-8ac3-def468e5e199",
        "description": "packaging in stock is a snapshot of full system balance to keep track over time.",
        "displayName": "Logistics Domain / Packaging Capability Entity / Packaging In Stock",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      },
      {
        "id": "29689083-c595-41fe-acc2-f1ab511e7693",
        "description": "packaging is the packaging used to protect parts sent to or from Scania. [Scania Lexicon - Part terms] Wrapping or container for goods. Material composition usually determined by the nature of the goods and even the mode of transport.[/]",
        "displayName": "Logistics Domain / Packaging Development Entity / Packaging",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      },
      {
        "id": "38278033-bdb7-4e98-a608-a643ff0da90a",
        "description": "packaging order is order for moving packaging between locations.",
        "displayName": "Logistics Domain / Packaging Planning Entity / Packaging Order",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      },
      {
        "id": "5f6f969d-a497-49ae-b06d-26eae5249851",
        "description": "packaging order line contains the information for each individual line in the packaging order.",
        "displayName": "Logistics Domain / Packaging Planning Entity / Packaging Order Line",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      },
      {
        "id": "93648f0b-1c9b-4251-a4f4-6953e0455858",
        "description": null,
        "displayName": "Logistics Domain / Physical product Packaging Entity / Packaging Instruction",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      },
      {
        "id": "350a9519-c2bb-4798-9615-d7364a4f4a86",
        "description": "shipment is the cohesive level to hold together a complete delivery between a part supplier and a consignee.",
        "displayName": "Logistics Domain / Transport Operation Entity / Shipment",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      },
      {
        "id": "89231c71-c3d7-427e-88ca-4831ad41a4b7",
        "description": "transport order is the activity that transfers objects between two places.",
        "displayName": "Logistics Domain / Transport Planning Entity / Transport Order",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      }
    ],
    "interfaceConsume": [
      {
        "id": "5cfdfc86-4528-4b0a-a15a-dc3ec7f2e707",
        "description": "API for Nile Packaging  - preprod environment",
        "displayName": "Nile_NilePreprodAPI IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-12-02"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "4d8678a8-91c5-4228-a823-56bc3b024619",
        "description": null,
        "displayName": "Nile_NIlePreprodStrAPI IE 1.0",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-03-14"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "022e5dca-4330-4990-bb8f-95d278c6db26",
        "description": "API for Nile Packaging  - prod environment",
        "displayName": "Nile_NileProdAPI IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-03-11"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "6ad913f0-a20a-4b69-854c-11dbe2d2123c",
        "description": null,
        "displayName": "PackIT_EmbasyPCService IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-04-16"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "6e813e75-eaeb-4889-ae2b-60640e148175",
        "description": null,
        "displayName": "PackIT_Packaging Instruction Master List",
        "lifecycle": null,
        "tags": []
      }
    ],
    "interfaceProvide": []
  },
  "tech": {
    "itComponents": {
      "software": [
        {
          "id": "8b6a0095-252e-40b2-8dc2-532307fc0269",
          "displayName": "WSO2 API Manager 3.2.0",
          "category": "software",
          "description": "WSO2 API Manager is a fully open-source solution for end to end API Management in the cloud, on-prem or in hybrid environments. It comes with an Apache Software License Version 2.0 which makes it free to use. It allows API developers to design, publish, and manage the lifecycle of APIs and API product managers to create API products from one or more APIs. It hosts an application developer portal which helps in building and managing a developer community for your APIs. Its cloud-native API gateway is used for securing, routing, controlling and monitoring your API traffic in a scalable manner.",
          "type": "ITComponent",
          "lifecycle": {
            "asString": "active",
            "phases": [
              {
                "phase": "active",
                "startDate": "2020-09-07"
              },
              {
                "phase": "endOfLife",
                "startDate": "2023-09-07"
              }
            ]
          },
          "tags": [
            "China affected"
          ]
        }
      ],
      "service": [
        {
          "id": "c8becefb-c450-41f4-b5e1-1a49c4b92066",
          "displayName": "Amazon.com / AWS EC2",
          "category": "service",
          "description": "Amazon EC2 presents a true virtual computing environment, allows to use web service interfaces to launch instances with a variety of operating systems, load them with custom application environment, manage network’s access permissions, and run image using as many or few systems as desire.",
          "type": "ITComponent",
          "lifecycle": {
            "asString": "active",
            "phases": [
              {
                "phase": "phaseIn",
                "startDate": "2006-08-24"
              },
              {
                "phase": "active",
                "startDate": "2008-10-23"
              }
            ]
          },
          "tags": [
            "China affected"
          ]
        },
        {
          "id": "c6b9f7b1-411f-445b-84c2-cb17391703b7",
          "displayName": "Aws DCE",
          "category": "service",
          "description": null,
          "type": "ITComponent",
          "lifecycle": null,
          "tags": [
            "China affected"
          ]
        },
        {
          "id": "3b450198-9c58-445d-93ab-06ee91e90439",
          "displayName": "Hosting of Datalake for Scania Sverige",
          "category": "service",
          "description": null,
          "type": "ITComponent",
          "lifecycle": null,
          "tags": [
            "China affected"
          ]
        }
      ],
      "accessControl": []
    },
    "ssoProvider": "",
    "statusSSO": "notSupported"
  },
  "chinaInfo": {
    "ChinaITlegalCompliance": "GapsFound",
    "ExportControlOfAppl": null,
    "LicenseUsageRights": null,
    "LicenceStatus": null,
    "ChinaRequirements": "OSPA",
    "ChinaDesign": "INAD/OSPA",
    "ChinaSourcing": "INAD",
    "ChinaPaying": "OSPA",
    "ChinaImplementation": "INAD",
    "Chinaverification": "INAD/OSPA",
    "ChinaOperations": "INAD"
  }
}