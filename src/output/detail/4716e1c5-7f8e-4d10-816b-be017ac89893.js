export default {
  "completion": {
    "percentage": 50,
    "ChinaReadiness": 0,
    "EnterpriseGuardrails": 50,
    "businessSupport": 55,
    "dataManagement": 25,
    "dependencies": 100,
    "information": 83,
    "projects": 100,
    "robots": 0,
    "sourcing": 36
  },
  "subscriptions": [
    {
      "firstName": "Kristofer",
      "lastName": "Enelund",
      "displayName": "Kristofer Enelund",
      "email": "kristofer.enelund@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "Information Owner"
      ]
    },
    {
      "firstName": "Ragini Sabharaj",
      "lastName": "Singh",
      "displayName": "Ragini Sabharaj Singh",
      "email": "ragini.singh@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "IT Delivery Responsible"
      ]
    },
    {
      "firstName": "Magnus",
      "lastName": "Lundin",
      "displayName": "Magnus Lundin",
      "email": "magnus.lundin@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "IT Delivery Responsible"
      ]
    },
    {
      "firstName": "Johan",
      "lastName": "Berggren",
      "displayName": "Johan Berggren",
      "email": "johan.berggren@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "IT Delivery Responsible"
      ]
    },
    {
      "firstName": "Anju",
      "lastName": "Naveenraj",
      "displayName": "Anju Naveenraj",
      "email": "anju.x.naveenraj@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "Developer"
      ]
    },
    {
      "firstName": "Andrea",
      "lastName": "Lingmar Rook",
      "displayName": "Andrea Lingmar Rook",
      "email": "andrea.lingmar.rook@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "Developer"
      ]
    },
    {
      "firstName": "Faisal",
      "lastName": "Ahmed",
      "displayName": "Faisal Ahmed",
      "email": "faisal.ahmed@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "Architect"
      ]
    }
  ],
  "information": {
    "name": "Your Next Truck",
    "description": "Your next truck is a marketing application that suggest and provides the customer with an appetiser how the customers next truck could look like.",
    "lifecycle": {
      "asString": "phaseIn",
      "phases": [
        {
          "phase": "phaseIn",
          "startDate": "2021-10-01",
          "milestoneId": null
        }
      ]
    },
    "productCategory": "",
    "predecessor": []
  },
  "dependencies": {
    "parent": [],
    "children": []
  },
  "initiatives": [],
  "businessSupport": {
    "functionFit": {
      "businessCriticality": "businessOperational",
      "functionalSuitability": "appropriate",
      "iramNecessary": "yes",
      "lifecycle": [
        {
          "asString": "iramCompleted",
          "phases": [
            {
              "phase": "iramCompleted",
              "startDate": "2022-01-27",
              "milestoneId": null
            },
            {
              "phase": "iramReview",
              "startDate": "2025-01-27",
              "milestoneId": null
            }
          ]
        },
        null
      ]
    },
    "capabilities": [
      {
        "id": "08f1f78a-8a79-40ea-858a-b6490b77b6d2",
        "displayName": "I. Presales, Sales and Customer Relationship Management",
        "description": "Ability to manage customer relations and steer presales and sales activities. Ability to reach the customers via resellers and partners. Ability to configure complete solutions to meet customer needs.",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "phaseIn",
              "startDate": "2022-07-01"
            },
            {
              "phase": "active",
              "startDate": "2022-10-05"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "MilkyWay area 17. Sales & Marketing"
        ]
      }
    ],
    "processes": [
      {
        "id": "95c6de4b-d409-4fea-975a-b46031c4c125",
        "displayName": "Sustainable Transport Solution / Sales",
        "description": "Sales Core Process consolidates sub-processes that are responsible to manage the output from the Product Development Core Process and prepare the input to both Order to Delivery and Services Delivery Core Processes. The sub-processes belonging to Sales Core Process are: Presales, Solution Sales, Rental and Used Vehicles Management.",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "plan",
              "startDate": "2018-01-01"
            },
            {
              "phase": "phaseIn",
              "startDate": "2019-01-01"
            },
            {
              "phase": "active",
              "startDate": "2019-01-01"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "Core process"
        ]
      },
      {
        "id": "fe3d4a1f-f617-4088-9dad-dc90b6ea19e8",
        "displayName": "Sustainable Transport Solution / Sales / Presales",
        "description": "Presales process is the consolidation of activities to be performed in preparation for Sales, before identification and contacting of potential buyers take place.",
        "lifecycle": {
          "asString": "plan",
          "phases": [
            {
              "phase": "plan",
              "startDate": "2020-10-01"
            }
          ]
        },
        "subscriptions": [],
        "tags": []
      }
    ],
    "organisations": [
      {
        "id": "ee543163-3216-4c67-9a37-de40b734004b",
        "displayName": "Scania - Scania CV AB / I - Scania IT / IA - Sales & Services IT / IAS - Sales IT / IASA - Customer Digitalization",
        "usageType": "ITmaintenance",
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-04-20"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "From IGA"
        ]
      },
      {
        "id": "f2089209-c3ee-45b1-94a5-dbe62573b2a7",
        "displayName": "Scania - Scania CV AB / P_SM - Sales & Marketing / KJ - Presales & Sales Readiness",
        "usageType": "owner",
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2020-09-18"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "From IGA"
        ]
      }
    ]
  },
  "management": {
    "objects": [],
    "interfaceConsume": [
      {
        "id": "a386abc8-bafe-4d72-9095-912ac4fe9dcc",
        "description": null,
        "displayName": "Explore Your Scania_TransformersTranslation IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-08-29"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "fefc2ef3-212b-42a9-a6eb-18e69a987841",
        "description": null,
        "displayName": "Image Service_ScaniaImageService IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-02-19"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "cf1db5c3-f379-475b-b875-1919448d7181",
        "description": null,
        "displayName": "Lead Import Tool_leadimport-backend IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-06-16"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "1d3a3387-e9b4-4508-b15c-85468018f594",
        "description": "API is responsible for receiving leads from Scania Configurator and AEM forms, and exposing them for CRM systems across Scania.",
        "displayName": "Lead Service_LeadService IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-03-08"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "c60cf4b0-e136-4d9d-b844-c3b3fba6a2a7",
        "description": "This are all external API(s) from Scania Configurator that needs to be consumed by Scania internal applications",
        "displayName": "Scania Configurator_ScaniaConfigurator IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-05-27"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "b601ad70-9249-442d-91a1-a3340d7e50e1",
        "description": "This service provides dealer, sales office, and workshop information.\n\nA dealer is an organization within the Scania authorized distribution network of products and services, and that is acting as both a sales office and a workshop.\n\nA sales office is an organization within the Scania authorized distribution network of products and services, in charge of sales and distribution of new Scania products and after-sales services. The facility includes a sales office where the sales force is stationed and where the customer can meet a salesman regularly. \n\nA workshop is an organization within the Scania distribution network of products and services, exclusively in charge of after-sales services. The network member has or is about to, undergo an audit according to DOS* to be certified and become a full member of the Scania network.",
        "displayName": "SIS_DealerService IE V2",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-03-04"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "114b45d0-6490-4622-8909-be0fec3cde1f",
        "description": "This service provides Distributor information.\n\nA distributor is an organization that is a member of the Scania distribution network of products and services set up by Scania CV AB and that is in charge of the wholesales and the distribution network for one or more Scania products and services in one or more countries.",
        "displayName": "SIS_DistributorService IE V1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-03-09"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "4ff88f7b-6b96-4ac3-9c80-42cd810c3517",
        "description": "API to get YNT details",
        "displayName": "Your Next Truck_Get-YNT-Details IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-11-18"
            }
          ]
        },
        "tags": []
      }
    ],
    "interfaceProvide": [
      {
        "id": "4ff88f7b-6b96-4ac3-9c80-42cd810c3517",
        "description": "API to get YNT details",
        "displayName": "Your Next Truck_Get-YNT-Details IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-11-18"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "8d1559c6-c41c-4b85-b49e-cf6d441deca5",
        "description": "YNT API",
        "displayName": "Your Next Truck_YNT IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-06-08"
            }
          ]
        },
        "tags": []
      },
      {
        "id": "59acf540-387b-4338-9962-34f504b4aa9b",
        "description": "This is admin API in YNT for creating custom mapping",
        "displayName": "Your Next Truck_YNT-Admin IE v1",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-11-08"
            }
          ]
        },
        "tags": []
      }
    ]
  },
  "tech": {
    "itComponents": {
      "software": [],
      "service": [
        {
          "id": "dde9e896-bc6f-4e90-8e12-36e633b47505",
          "displayName": "Amazon.com / AWS DynamoDB",
          "category": "service",
          "description": "DynamoDB is a fast and flexible nonrelational database service for any scale. DynamoDB enables customers to offload the administrative burdens of operating and scaling distributed databases to AWS so that they don’t have to worry about hardware provisioning, setup and configuration, throughput capacity planning, replication, software patching, or cluster scaling.",
          "type": "ITComponent",
          "lifecycle": {
            "asString": "active",
            "phases": [
              {
                "phase": "active",
                "startDate": "2012-01-18"
              }
            ]
          },
          "tags": [
            "China affected"
          ]
        }
      ],
      "accessControl": []
    },
    "ssoProvider": "",
    "statusSSO": null
  },
  "chinaInfo": {
    "ChinaITlegalCompliance": null,
    "ExportControlOfAppl": null,
    "LicenseUsageRights": null,
    "LicenceStatus": null,
    "ChinaRequirements": "",
    "ChinaDesign": "",
    "ChinaSourcing": "",
    "ChinaPaying": "",
    "ChinaImplementation": "",
    "Chinaverification": "",
    "ChinaOperations": ""
  }
}