export default {
  "completion": {
    "percentage": 75,
    "ChinaReadiness": 8,
    "EnterpriseGuardrails": 100,
    "businessSupport": 68,
    "dataManagement": 66,
    "dependencies": 100,
    "information": 92,
    "projects": 100,
    "robots": 100,
    "sourcing": 46
  },
  "subscriptions": [
    {
      "firstName": "Palle",
      "lastName": "Berlin",
      "displayName": "Palle Berlin",
      "email": "palle.berlin@tratonfs.com",
      "type": "RESPONSIBLE",
      "role": [
        "Product Owner"
      ]
    },
    {
      "firstName": "Danilo",
      "lastName": "Oliveira",
      "displayName": "Danilo Oliveira",
      "email": "danilo.oliveira@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "IT Delivery Responsible"
      ]
    },
    {
      "firstName": "Ulrika",
      "lastName": "Widén",
      "displayName": "Ulrika Widén",
      "email": "ulrika.widen@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "IT Delivery Responsible"
      ]
    }
  ],
  "information": {
    "name": "CRAM",
    "description": "Supports the Credit Process with Expected Credit Loss information and other related calculations. \nObjective of CRAM Credit Risk & Financial Asset Management is to operationalize Scania Financial Services Accounting process as specified by the IFRS9 - International Accounting Standards Board using an COTS IT Platform SAS 9.4",
    "lifecycle": {
      "asString": "active",
      "phases": [
        {
          "phase": "phaseIn",
          "startDate": "2018-05-02",
          "milestoneId": null
        },
        {
          "phase": "active",
          "startDate": "2023-04-28",
          "milestoneId": null
        }
      ]
    },
    "productCategory": "",
    "predecessor": [
      {
        "id": "da13a23f-7994-4978-bd3b-8d2e4df3cbff",
        "displayName": "Office 365 / Excel",
        "description": "Software for creating and editing spreadsheets",
        "type": "Application",
        "tags": [
          "China affected",
          "Maturity Level 2"
        ],
        "subscriptions": []
      }
    ],
    "successor": []
  },
  "dependencies": {
    "parent": [],
    "children": []
  },
  "initiatives": [
    {
      "id": "213a83be-53eb-4439-8bb8-33ef1fd1de95",
      "displayName": "CRAM",
      "fullName": "CRAM",
      "tags": [
        "Maturity Level 2"
      ],
      "lifecycle": {
        "asString": "plan",
        "phases": [
          {
            "phase": "plan",
            "startDate": "2018-01-01"
          },
          {
            "phase": "endOfLife",
            "startDate": "2025-04-30"
          }
        ]
      },
      "subscriptions": [
        {
          "displayName": "Pawel Hallgren",
          "email": "pawel.hallgren@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Mattias Lindskog",
          "email": "mattias.lindskog@scania.com",
          "type": "RESPONSIBLE"
        }
      ]
    },
    {
      "id": "7f33ee56-c6a9-4858-8719-fea2510e7232",
      "displayName": "UNITY (FS)",
      "fullName": "UNITY (FS)",
      "tags": [
        "Maturity Level 2",
        "IMG Status Initiatives"
      ],
      "lifecycle": {
        "asString": "active",
        "phases": [
          {
            "phase": "active",
            "startDate": "2019-10-15"
          }
        ]
      },
      "subscriptions": [
        {
          "displayName": "Sirpa Södereng",
          "email": "sirpa.sodereng@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Michael Öhman",
          "email": "michael.ohman@tratonfs.com",
          "type": "RESPONSIBLE"
        }
      ]
    }
  ],
  "businessSupport": {
    "functionFit": {
      "businessCriticality": "businessCritical",
      "functionalSuitability": "perfect",
      "iramNecessary": "yes",
      "lifecycle": [
        {
          "asString": "iramReview",
          "phases": [
            {
              "phase": "iramCompleted",
              "startDate": "2019-08-23",
              "milestoneId": null
            },
            {
              "phase": "iramReview",
              "startDate": "2022-08-23",
              "milestoneId": null
            }
          ]
        },
        null
      ]
    },
    "capabilities": [
      {
        "id": "fae8866f-b277-4b5a-82af-ad4c669856e6",
        "displayName": "N. Financial Planning and Accounting / Accounting & Financial Close / Financial Close",
        "description": "This capability describes all required period end-closing steps, related to performing local period-end closing and steps that are identical for local and parallel ledger.",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "plan",
              "startDate": "2021-10-12"
            },
            {
              "phase": "phaseIn",
              "startDate": "2021-10-17"
            },
            {
              "phase": "active",
              "startDate": "2021-10-17"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "China affected"
        ]
      }
    ],
    "processes": [
      {
        "id": "e8de1dbb-bdae-4f49-b264-d0f0f2063b2a",
        "displayName": "Supporting processes / Finance Process / Financial Services",
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "plan",
              "startDate": "2020-01-22"
            },
            {
              "phase": "phaseIn",
              "startDate": "2020-01-22"
            },
            {
              "phase": "active",
              "startDate": "2020-01-22"
            }
          ]
        },
        "subscriptions": [],
        "tags": []
      },
      {
        "id": "cda09350-a327-4089-8d42-b36bda8a5ed4",
        "displayName": "Supporting processes / Finance Process / Group Reporting",
        "description": null,
        "lifecycle": {
          "asString": "plan",
          "phases": [
            {
              "phase": "plan",
              "startDate": "2020-01-22"
            }
          ]
        },
        "subscriptions": [],
        "tags": []
      }
    ],
    "organisations": [
      {
        "id": "953751fc-331d-41a5-9563-292a9401f740",
        "displayName": "Scania - Scania CV AB / I - Scania IT / IZ - IZ Digital Core / IZD - Financial services IT",
        "usageType": "ITmaintenance",
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2022-05-01"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "From IGA"
        ]
      },
      {
        "id": "ca2c980b-0cf0-4af4-a133-fca5a6b66782",
        "displayName": "Scania - Scania CV AB / P_FB - Finance and Business Control / F - Financial Services",
        "usageType": "owner",
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2019-04-26"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "From IGA"
        ]
      }
    ]
  },
  "management": {
    "objects": [
      {
        "id": "4dbb98f8-e583-4368-9ce4-8c0f67379b96",
        "description": "A ledger account is a record in an accounting system that tracks the financial activities of a specific asset, liability, equity, revenue, or expense.",
        "displayName": "Financial Control Domain / Ledger and Account Entity / Ledger Account",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      },
      {
        "id": "5a0ad18b-51b6-4a5c-ad91-11428d93cc0f",
        "description": "Scania Financial Services (SFS) is serving Scania customers, dealers and distributors in 50 markets through 15 business units (BU’s) by offering competitive and innovative financing",
        "displayName": "Organisation Domain / Party & Customer Entity / Financial Services",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      }
    ],
    "interfaceConsume": [
      {
        "id": "1cf3ab84-f8b6-4d2f-b742-33929bd91899",
        "description": null,
        "displayName": "Currency File",
        "lifecycle": null,
        "tags": []
      }
    ],
    "interfaceProvide": [
      {
        "id": "601f96e1-6210-4416-a254-9179ec41b4b4",
        "description": null,
        "displayName": "CRAM_HFM Result",
        "lifecycle": null,
        "tags": []
      },
      {
        "id": "3674b140-5306-49fb-89cb-64d839310420",
        "description": "Loss allowance interface contains data for Customer ID, Contract ID, Amount, loss allowance.",
        "displayName": "CRAM_Loss Allowance [Manual - FileHandler]",
        "lifecycle": null,
        "tags": [
          "Not China affected"
        ]
      },
      {
        "id": "bffd86ff-7548-416e-a116-080d0d6f5b7f",
        "description": "Loss-Allowance amounts (aka Bad Debt Reserves) with details such as Expected Credit Loss (ECL). Contract-level granularity.",
        "displayName": "CRAM_Loss Allowance [Manual - FILESHARE]",
        "lifecycle": null,
        "tags": []
      },
      {
        "id": "59cc18e0-b3ba-4e85-9ab6-0b166ae016d1",
        "description": null,
        "displayName": "CRAM_Rental_Data",
        "lifecycle": null,
        "tags": []
      }
    ]
  },
  "tech": {
    "itComponents": {
      "software": [
        {
          "id": "13df2746-a572-41ad-bfc5-7e862de64509",
          "displayName": "EnterpriseDB PostgreSQL 9.4",
          "category": "software",
          "description": "PostgreSQL is an open source object-relational database platform with over 30 years of active development. As an enterprise-ready platform, Postgres offers extensibility and reliability in both data warehousing and high-volume online transaction processing systems; specialized support for many programming languages and a proven track record for high availability, failover capability, and point-in-time recovery",
          "type": "ITComponent",
          "lifecycle": {
            "asString": "endOfLife",
            "phases": [
              {
                "phase": "active",
                "startDate": "2014-12-18"
              },
              {
                "phase": "endOfLife",
                "startDate": "2020-02-13"
              }
            ]
          },
          "tags": [
            "China affected"
          ]
        },
        {
          "id": "f0a4cf90-c322-4c27-9648-b973a1cf1c16",
          "displayName": "Oracle Java Runtime Environment Standard 8",
          "category": "software",
          "description": "The Java Runtime Environment, or JRE, is a software layer that runs on top of a computer’s operating system software and provides the class libraries and other resources that a specific Java program needs to run.",
          "type": "ITComponent",
          "lifecycle": {
            "asString": "phaseOut",
            "phases": [
              {
                "phase": "active",
                "startDate": "2014-03-01"
              },
              {
                "phase": "phaseOut",
                "startDate": "2022-03-31"
              },
              {
                "phase": "endOfLife",
                "startDate": "2030-12-31"
              }
            ]
          },
          "tags": [
            "China affected",
            "From SCT"
          ]
        },
        {
          "id": "69d68068-2cfc-44d7-ad2e-5fd0141938ad",
          "displayName": "Red Hat Enterprise Linux 7.9",
          "category": "software",
          "description": "Red Hat Enterprise Linux is an open-source operating system. It enables users to scale existing apps and roll out emerging technologies across bare-metal, virtual, container, and cloud environments.",
          "type": "ITComponent",
          "lifecycle": {
            "asString": "active",
            "phases": [
              {
                "phase": "active",
                "startDate": "2020-09-29"
              },
              {
                "phase": "endOfLife",
                "startDate": "2024-06-30"
              }
            ]
          },
          "tags": [
            "China affected",
            "From SCT"
          ]
        }
      ],
      "service": [],
      "accessControl": []
    },
    "ssoProvider": "",
    "statusSSO": "notSupported"
  },
  "chinaInfo": {
    "ChinaITlegalCompliance": "SQCcompleted",
    "ExportControlOfAppl": null,
    "LicenseUsageRights": null,
    "LicenceStatus": null,
    "ChinaRequirements": "",
    "ChinaDesign": "",
    "ChinaSourcing": "",
    "ChinaPaying": "",
    "ChinaImplementation": "",
    "Chinaverification": "",
    "ChinaOperations": ""
  }
}