export default {
  "completion": {
    "percentage": 76,
    "ChinaReadiness": 8,
    "EnterpriseGuardrails": 100,
    "businessSupport": 70,
    "dataManagement": 86,
    "dependencies": 50,
    "information": 96,
    "projects": 100,
    "robots": 100,
    "sourcing": 70
  },
  "subscriptions": [
    {
      "firstName": null,
      "lastName": null,
      "displayName": "jan.erlandsson@scania.com",
      "email": "jan.erlandsson@scania.com",
      "type": "OBSERVER",
      "role": [
        "Expert User"
      ]
    },
    {
      "firstName": null,
      "lastName": null,
      "displayName": "ulf.oberg@scania.com",
      "email": "ulf.oberg@scania.com",
      "type": "OBSERVER",
      "role": [
        "Expert User"
      ]
    },
    {
      "firstName": "Roger",
      "lastName": "Axelsson",
      "displayName": "Roger Axelsson",
      "email": "roger.axelsson@scania.com",
      "type": "OBSERVER",
      "role": [
        "Expert User"
      ]
    },
    {
      "firstName": null,
      "lastName": null,
      "displayName": "bjorn.baecklund@scania.com",
      "email": "bjorn.baecklund@scania.com",
      "type": "OBSERVER",
      "role": [
        "Expert User"
      ]
    },
    {
      "firstName": "Mikael",
      "lastName": "Rothsten",
      "displayName": "Mikael Rothsten",
      "email": "mikael.rothsten@scania.com",
      "type": "OBSERVER",
      "role": [
        "Expert User"
      ]
    },
    {
      "firstName": null,
      "lastName": null,
      "displayName": "stefan.myrberg@scania.com",
      "email": "stefan.myrberg@scania.com",
      "type": "OBSERVER",
      "role": [
        "Expert User"
      ]
    },
    {
      "firstName": "Anders",
      "lastName": "Asplund",
      "displayName": "Anders Asplund",
      "email": "anders.asplund@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "IT Delivery Responsible"
      ]
    },
    {
      "firstName": "Viktor",
      "lastName": "Kaznov",
      "displayName": "Viktor Kaznov",
      "email": "viktor.kaznov@scania.com",
      "type": "OBSERVER",
      "role": []
    },
    {
      "firstName": "Henrik",
      "lastName": "Linder",
      "displayName": "Henrik Linder",
      "email": "henrik.linder@scania.com",
      "type": "OBSERVER",
      "role": []
    },
    {
      "firstName": "Stefan",
      "lastName": "Kull",
      "displayName": "Stefan Kull",
      "email": "stefan.kull@scania.com",
      "type": "OBSERVER",
      "role": []
    },
    {
      "firstName": "Daniel",
      "lastName": "Kastensson",
      "displayName": "Daniel Kastensson",
      "email": "daniel.kastensson@scania.com",
      "type": "RESPONSIBLE",
      "role": []
    },
    {
      "firstName": "Erik",
      "lastName": "Comstedt",
      "displayName": "Erik Comstedt",
      "email": "erik.comstedt@scania.com",
      "type": "RESPONSIBLE",
      "role": []
    },
    {
      "firstName": "Eva",
      "lastName": "Frykevall",
      "displayName": "Eva Frykevall",
      "email": "eva.frykevall@scania.com",
      "type": "RESPONSIBLE",
      "role": []
    },
    {
      "firstName": "Andreas",
      "lastName": "Henriksson",
      "displayName": "Andreas Henriksson",
      "email": "andreas.henriksson@scania.com",
      "type": "RESPONSIBLE",
      "role": []
    },
    {
      "firstName": "Jonathan",
      "lastName": "Koitsalu",
      "displayName": "Jonathan Koitsalu",
      "email": "jonathan.koitsalu@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "Developer"
      ]
    },
    {
      "firstName": "Ewa-Lena",
      "lastName": "Rasmusson",
      "displayName": "Ewa-Lena Rasmusson",
      "email": "ewa-lena.rasmusson@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "IT Delivery Responsible"
      ]
    },
    {
      "firstName": "Georg",
      "lastName": "Grüneissl",
      "displayName": "Georg Grüneissl",
      "email": "georg.gruneissl@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "Information Owner"
      ]
    },
    {
      "firstName": "Magnus",
      "lastName": "Milstig",
      "displayName": "Magnus Milstig",
      "email": "magnus.milstig@scania.com",
      "type": "RESPONSIBLE",
      "role": []
    },
    {
      "firstName": "Mattias",
      "lastName": "Wemmenborn",
      "displayName": "Mattias Wemmenborn",
      "email": "mattias.wemmenborn@scania.com",
      "type": "RESPONSIBLE",
      "role": [
        "Product Owner"
      ]
    }
  ],
  "information": {
    "name": "SesammTool 2",
    "description": "Sesamm Tool and Sesamm Tool 2",
    "lifecycle": {
      "asString": "active",
      "phases": [
        {
          "phase": "active",
          "startDate": "2021-12-06",
          "milestoneId": null
        }
      ]
    },
    "productCategory": "Development Tool",
    "predecessor": [
      {
        "id": "c31815d1-fef0-4e7b-830e-ab2e1225dd08",
        "displayName": "Sesamm Tool 1",
        "description": null,
        "type": "Application",
        "tags": [],
        "subscriptions": []
      }
    ],
    "successor": [
      {
        "id": "0d74052e-caf9-42db-bbdc-6954fec32f15",
        "displayName": "TEESA Tool",
        "description": null,
        "type": "Application",
        "tags": [],
        "subscriptions": []
      }
    ]
  },
  "dependencies": {
    "parent": [],
    "children": [
      {
        "id": "60a8253b-68fc-4605-9ef3-febc70e01179",
        "activeFrom": "2021-12-06",
        "activeUntil": null,
        "description": "MSC(Message Sequence Charts) The MSC describes the signal flow between the environment, external sensors, the driver, ECU:s and actuators that is necessary for a function to be executed on a time line. \n",
        "displayName": "SesammTool 2 / MSCDRAWER",
        "type": "Application",
        "lifecycle": null,
        "tags": [
          "Maturity Level 1"
        ],
        "subscriptions": []
      }
    ]
  },
  "initiatives": [
    {
      "id": "86db40ea-4776-4af2-933a-d35af862c095",
      "displayName": "SAS Industrial Project - China",
      "fullName": "SAS Industrial Project - China",
      "tags": [
        "China affected"
      ],
      "lifecycle": {
        "asString": "active",
        "phases": [
          {
            "phase": "active",
            "startDate": "2021-09-01"
          }
        ]
      },
      "subscriptions": [
        {
          "displayName": "Anton Smith",
          "email": "anton.smith@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Enno Grüning",
          "email": "enno.gruning@scania.com",
          "type": "OBSERVER"
        }
      ]
    },
    {
      "id": "b0338880-cb96-439d-85c1-e3571347754d",
      "displayName": "SAS Industrial Project - China / China RnD PrIT",
      "fullName": "China RnD PrIT",
      "tags": [
        "China affected"
      ],
      "lifecycle": null,
      "subscriptions": [
        {
          "displayName": "Erik Eriksson",
          "email": "erik.x.eriksson@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Marcus Gustafsson",
          "email": "marcus_x.gustafsson@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Joanna Pochec",
          "email": "joanna.pochec@scania.com",
          "type": "OBSERVER"
        }
      ]
    },
    {
      "id": "6bf7bf52-7793-48fe-856a-3f1e3f096fe2",
      "displayName": "UNECE / Cyber Security Management System (CSMS)",
      "fullName": "Cyber Security Management System (CSMS)",
      "tags": [],
      "lifecycle": null,
      "subscriptions": [
        {
          "displayName": "Torgrim Oldenburg",
          "email": "torgrim.oldenburg@scania.com",
          "type": "RESPONSIBLE"
        },
        {
          "displayName": "Ioannis Tzioumakas",
          "email": "ioannis.tzioumakas@scania.com",
          "type": "OBSERVER"
        }
      ]
    }
  ],
  "businessSupport": {
    "functionFit": {
      "businessCriticality": "businessCritical",
      "functionalSuitability": "appropriate",
      "iramNecessary": "yes",
      "lifecycle": [
        {
          "asString": "iramCompleted",
          "phases": [
            {
              "phase": "iramCompleted",
              "startDate": "2022-09-21",
              "milestoneId": null
            },
            {
              "phase": "iramReview",
              "startDate": "2025-09-21",
              "milestoneId": null
            }
          ]
        },
        null
      ]
    },
    "capabilities": [
      {
        "id": "f6bd289b-6f5a-4b4e-8de7-f46c52b5dc41",
        "displayName": "E. Solution Development & Optimization / Design and Maintain Modular Architecture",
        "description": "The ability to manage different structures of Scania products and services including structures for Development, Sales and Manufacturing.",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2018-09-13"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "China affected",
          "MilkyWay area 7. Manage Change, Configuration and Releases"
        ]
      },
      {
        "id": "098381b7-d25e-40e8-bf19-7cf55659c744",
        "displayName": "E. Solution Development & Optimization / Transport Solution Design / Design Embedded and Electronics Systems",
        "description": "Design and develop embedded systems, including software and hardware. Designing embedded & electrical schematics, cable harnesses including chassis installation, connectors and passive elements.",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2018-09-13"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "China affected"
        ]
      }
    ],
    "processes": [
      {
        "id": "035769ce-9590-4542-8ae2-d3d50a5a98f4",
        "displayName": "Product development / R&D Process / Embedded System process / 04 Architecture",
        "description": null,
        "lifecycle": null,
        "subscriptions": [],
        "tags": []
      },
      {
        "id": "e2751da9-584a-4b7e-ad5e-30f4fdbb3106",
        "displayName": "Product development / R&D Process / Embedded System process / 08 Production",
        "description": null,
        "lifecycle": null,
        "subscriptions": [],
        "tags": []
      }
    ],
    "organisations": [
      {
        "id": "65e6e56c-2a76-493a-ba81-89ee7ead3ede",
        "displayName": "Scania - Scania CV AB / P_RDP - R&D and Purchasing / EE - EE Software and Autonomous / EED - Software Development System / EEDI - Information Architecture and Tool Chain for Embedded Systems",
        "usageType": "owner",
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2023-07-14"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "From IGA"
        ]
      },
      {
        "id": "ef5338d9-dee4-43ab-a52d-f8e9853c3cd4",
        "displayName": "Scania - Scania CV AB / P_RDP - R&D and Purchasing / EE - EE Software and Autonomous / EED - Software Development System / EEDI - Information Architecture and Tool Chain for Embedded Systems / EEDIF - Embedded Process Tools Development F",
        "usageType": null,
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2023-07-14"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "From IGA"
        ]
      },
      {
        "id": "e4a0cb2d-3704-4bf6-aa7d-4d2b46311040",
        "displayName": "Scania - Scania CV AB / P_RDP - R&D and Purchasing / EE - EE Software and Autonomous / EEP - System Platform / EEPX - System Architecture / EEPXA - Systems Functional Architecture",
        "usageType": "user",
        "description": null,
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2023-07-14"
            }
          ]
        },
        "subscriptions": [],
        "tags": [
          "From IGA"
        ]
      }
    ]
  },
  "management": {
    "objects": [
      {
        "id": "60c3fd80-a5ca-406a-88de-6c6082223f21",
        "description": "Represents the underlying specification and carry through level that describes and log each activity that fulfil a Product Development Project.",
        "displayName": "Product Development Domain / Configuration Product Development Entity / Product Development Activity",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      },
      {
        "id": "773a1d70-9ff0-44df-93e7-a913112635b4",
        "description": "Specifies the content and items for product development, whole Product As Designed or built-in Component, with its various performance requirements which are part of the Product Development Project and that will be maintained in the project's activities.",
        "displayName": "Product Development Domain / Configuration Product Development Entity / Product Development Object",
        "lifecycle": {
          "asString": "active",
          "phases": [
            {
              "phase": "active",
              "startDate": "2021-09-08"
            }
          ]
        },
        "tags": [
          "CIM (Common Information Model)"
        ]
      }
    ],
    "interfaceConsume": [
      {
        "id": "c866762d-dcd3-4a5e-a4b5-d739a249bf8a",
        "description": null,
        "displayName": "Embedded Linked Data Enablement, SESAM-tool_Embedded Linked Data Enablement, SESAMM-tool - https://rdftest.scania.com/oas/",
        "lifecycle": null,
        "tags": []
      },
      {
        "id": "90b0d447-0974-4c65-a31d-03ac988f757d",
        "description": null,
        "displayName": "Lexicon Part Descriptions",
        "lifecycle": null,
        "tags": []
      },
      {
        "id": "a7dfc0be-cb70-4293-bd43-5df0543803f4",
        "description": null,
        "displayName": "MSC Drawer",
        "lifecycle": null,
        "tags": []
      },
      {
        "id": "cd3c8b0d-5c09-407c-9e5d-e367d17a6604",
        "description": null,
        "displayName": "OAS Platform / OAS_OAS Part Information",
        "lifecycle": null,
        "tags": []
      },
      {
        "id": "3c8a9aa2-d938-4aa2-a6d5-e2c028fd02de",
        "description": null,
        "displayName": "OAS Platform / OAS_OAS_FPCCodes_SCCO7788 [MQ]",
        "lifecycle": null,
        "tags": []
      },
      {
        "id": "c5708dda-80c8-4366-9f90-09ce245c6503",
        "description": null,
        "displayName": "SesammTool 2_Scania Lexicon",
        "lifecycle": null,
        "tags": []
      },
      {
        "id": "cffe8495-536c-4716-9afb-67955e4efa29",
        "description": null,
        "displayName": "WAM_JiraDMZ - https://issues.scania.com",
        "lifecycle": null,
        "tags": [
          "XDS_RP"
        ]
      }
    ],
    "interfaceProvide": [
      {
        "id": "fc46e925-d298-4fd3-9439-21bc5a6feca9",
        "description": null,
        "displayName": "SesammTool 2_jira",
        "lifecycle": null,
        "tags": []
      },
      {
        "id": "8261c236-7e36-4ae2-87bf-e36132b9417b",
        "description": null,
        "displayName": "SesammTool 2_Power Sesamm",
        "lifecycle": null,
        "tags": []
      },
      {
        "id": "c5708dda-80c8-4366-9f90-09ce245c6503",
        "description": null,
        "displayName": "SesammTool 2_Scania Lexicon",
        "lifecycle": null,
        "tags": []
      }
    ]
  },
  "tech": {
    "itComponents": {
      "software": [
        {
          "id": "f0674444-36de-427b-a6d7-879703d12ae1",
          "displayName": "Microsoft SQL Server Standard 2016",
          "category": "software",
          "description": "Microsoft SQL Server is a relational database management system. As a database server, it is a software product with the primary function of storing and retrieving data as requested by other software applications.",
          "type": "ITComponent",
          "lifecycle": {
            "asString": "endOfLife",
            "phases": [
              {
                "phase": "phaseIn",
                "startDate": "2016-03-07"
              },
              {
                "phase": "active",
                "startDate": "2016-06-01"
              },
              {
                "phase": "endOfLife",
                "startDate": "2018-01-09"
              }
            ]
          },
          "tags": [
            "From SCT"
          ]
        },
        {
          "id": "48c1b85b-a8f3-4f0f-8d18-6a47ec0ee11c",
          "displayName": "yFiles",
          "category": "software",
          "description": null,
          "type": "ITComponent",
          "lifecycle": null,
          "tags": [
            "China affected"
          ]
        }
      ],
      "service": [],
      "accessControl": []
    },
    "ssoProvider": "Global ID",
    "statusSSO": "supported"
  },
  "chinaInfo": {
    "ChinaITlegalCompliance": null,
    "ExportControlOfAppl": null,
    "LicenseUsageRights": null,
    "LicenceStatus": null,
    "ChinaRequirements": "",
    "ChinaDesign": "",
    "ChinaSourcing": "",
    "ChinaPaying": "",
    "ChinaImplementation": "",
    "Chinaverification": "",
    "ChinaOperations": ""
  }
}